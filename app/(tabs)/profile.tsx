...
export default function ProfileTab() {
  ...
  const handleRetry = () => {
    console.log('[Profile] Retry requested');
    setHasError(false);
    setIsLoading(true);
    
    // If we have a pending URL (from a navigation), reload to that
    if (pendingUrlRef.current && ref.current) {
      console.log('[Profile] Reloading to pending URL:', pendingUrlRef.current);
    ...
    <View style={styles.container}>
      <WebView
        ...
        onLoadStart={(syntheticEvent) => {
          ...
          console.log('[Profile] Load started:', nativeEvent.url);
          ...
          loadingTimeoutRef.current = setTimeout(() => {
            console.log('[Profile] Load timeout - showing retry button');
            setIsLoading(false);
            setRefreshing(false);
            setHasError(true);
          }, 20000);
        ...
        onError={(syntheticEvent) => {
          const { nativeEvent } = syntheticEvent;
          console.error('[Profile] WebView error:', nativeEvent);
          setIsLoading(false);
          setRefreshing(false);
        ...
        onHttpError={(syntheticEvent) => {
          const { nativeEvent } = syntheticEvent;
          console.error('[Profile] HTTP error:', nativeEvent.statusCode, nativeEvent.url);
          setIsLoading(false);
          setRefreshing(false);
          setHasError(true);
        }}
        ...
        injectedJavaScriptBeforeContentLoaded={`
          ...
          (function() {
            try {
              ...
              console.log('[Profile] Browser API patch error:', e);
            }
      ...
      {hasError && (
        <View style={styles.errorContainer}>
          ...
          <TouchableOpacity style={styles.retryButtonStyle} onPress={handleRetry}>
            <Text style={styles.retryButtonTextStyle}>Tap to Retry</Text>
          </TouchableOpacity>
        </View>
      )}
